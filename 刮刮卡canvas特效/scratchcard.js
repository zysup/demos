...
ScratchCard.prototype = {
  ...
  _scratch: function (e) {
    ...
    if (!this.done && this.isDown) {
      ...
      var x =
          (e.clientX + document.body.scrollLeft || e.pageX) - this.offsetX || 0,
        y = (e.clientY + document.body.scrollTop || e.pageY) - this.offsetY || 0
      with (this.ctx) {
        beginPath()
        arc(
          x * that.config.pixelRatio,
          y * that.config.pixelRatio,
          that.config.radius * that.config.pixelRatio,
          0,
          Math.PI * 2
        )
        fill()
  ...
  _scratchAll() {
    ...
    if (this.config.fadeOut > 0) {
      ...
      this.canvas.style.transition =
        'all ' + this.config.fadeOut / 1000 + 's linear'
